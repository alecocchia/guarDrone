...
tmux send-keys -t $SESSION_NAME:0.3 "sleep 10 && ros2 launch drone_mpc_pkg haptic_simulation.launch.py model:=${DRONE1_MODEL_NAME} drone_x:=${DRONE1_X} drone_y:=${DRONE1_Y} drone_z:=${DRONE1_Z} drone_yaw:=${DRONE1_YAW} peg_x:=${DRONE2_X} peg_y:=${DRONE2_Y} peg_z:=${DRONE2_Z}" C-m
