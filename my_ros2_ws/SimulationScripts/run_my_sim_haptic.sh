...
tmux send-keys -t $SESSION_NAME:0.2 "ros2 launch drone_mpc_pkg haptic_mpc.launch.py model:=${MODEL_NAME} world:=${WORLD_NAME}.sdf drone_x:=${DRONE_X} drone_y:=${DRONE_Y} drone_z:=${DRONE_Z} drone_yaw:=${DRONE_YAW}" C-m
