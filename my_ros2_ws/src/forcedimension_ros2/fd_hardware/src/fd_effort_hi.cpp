  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  if (it_interface_id != info_.hardware_parameters.end()) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "Using interface ID: %d", interface_ID_);
  } else {
    interface_ID_ = -1;
  }
  ...
  if (it_interface_serial_number != info_.hardware_parameters.end()) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "Using interface serial number: %d", interface_SN_);
  } else {
    interface_SN_ = -1;
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "Emulating button: %s", emulate_button_ ? "true" : "false");
  ...
  if (it_interface_mass != info_.hardware_parameters.end()) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "Interface mass parameter found: %lf Kg", effector_mass_);
  } else {
    effector_mass_ = -1.0;
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "Ignoring orientation readings: %s", ignore_orientation_ ? "true" : "false");
  ...
  const rclcpp_lifecycle::State & previous_state)
{
  ...
  if (connectToDevice()) {
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "System Successfully started!");
    return CallbackReturn::SUCCESS;
  } else {
    RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "System Not started!");
    return CallbackReturn::ERROR;
  }
}
// ------------------------------------------------------------------------------------------
  ...
  const rclcpp_lifecycle::State & previous_state)
{
  ...
  if (disconnectFromDevice()) {
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "System successfully stopped!");
    return CallbackReturn::SUCCESS;
  } else {
    RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "System Not stopped!");
    return CallbackReturn::ERROR;
  }
}
// ------------------------------------------------------------------------------------------
...
bool FDEffortHardwareInterface::connectToDevice()
{
  ...
  if (interface_SN_ >= 0) {
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Connecting to device with serial number %d, please wait...", interface_SN_);
    ...
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Connecting to device with ID= %d, please wait...", interface_ID_);
    interface_ID_ = dhdOpenID(interface_ID_);
    dhd_open_success = (interface_ID_ >= 0);
    // dhd_open_success = (dhdOpenID(interface_ID_) >= 0);
  } else {
    // Open default device
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Connecting to default device., please wait..");
  ...
  if (dhd_open_success) {
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : %s device detected", dhdGetSystemName(interface_ID_));
    uint16_t serialNumber = 0;
    if (dhdGetSerialNumber(&serialNumber, interface_ID_) < 0) {
      RCLCPP_WARN(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Impossible to retrieve device serial number: %s!",
        dhdErrorGetLastStr());
    } else {
      RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : device serial number = %d", serialNumber);
    }
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : device interface ID = %d", interface_ID_);

    // Check if the device has 3 dof or more
    if (dhdHasWrist(interface_ID_)) {
      RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Rotation supported");
    } else {
      RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Rotation not supported");
    }
    ...
    if (dhdGetEffectorMass(&current_effector_mass, interface_ID_) == DHD_NO_ERROR) {
      RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Effector Mass = %f (g)", current_effector_mass * 1000.0);
    } else {
    ...
    if (dhdSetMaxForce(forceMax, interface_ID_) < DHD_NO_ERROR) {
      RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Could not set max force!");
    ...
    if (dhdEnableForce(DHD_ON, interface_ID_) < DHD_NO_ERROR) {
      RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Could not enable force control!");
      disconnectFromDevice();
      return false;
    }
    // Set effector mass
    if (effector_mass_ > 0.0) {
      ...
      if (dhdSetEffectorMass(effector_mass_, interface_ID_) < DHD_NO_ERROR) {
        RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Failed to set effector mass!");
        disconnectFromDevice();
        return false;
      }
    }
    ...
    if (dhdSetGravityCompensation(DHD_ON, interface_ID_) < DHD_NO_ERROR) {
      RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Could not enable the gravity compensation !");
      disconnectFromDevice();
      return false;
    } else {
    ...
    if (emulate_button_ && !dhdHasGripper(interface_ID_)) {
      RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Could not enable button emulation, no gripper found!");
    } else if (emulate_button_ && dhdHasGripper(interface_ID_)) {
      RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Emulating button from clutch joint...");
      if (dhdEmulateButton(DHD_ON, interface_ID_) < DHD_NO_ERROR) {
        RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Could not enable button emulation!");
        disconnectFromDevice();
        return false;
      }
      RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : OK, button will be emulated from clutch joint.");
    }
    // Set force to zero
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Could not initialize force control!");
      disconnectFromDevice();
      return false;
    }
    ...
    if (ignore_orientation_) {
      RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : Orientation will be ignored !");
    }
...
bool FDEffortHardwareInterface::disconnectFromDevice()
{
  ...
  int hasStopped = -1;
  while (hasStopped < 0) {
    RCLCPP_INFO(rclcpp::get_logger("FDEffortHardwareInterface"), "dhd : stopping the device, please wait...");
